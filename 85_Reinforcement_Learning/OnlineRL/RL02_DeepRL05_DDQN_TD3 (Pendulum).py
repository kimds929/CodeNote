...
# Critic Network
critic_main1_network = Critic(state_dim=obs_dim, action_dim=action_dim, hidden_dim=64).to(device)
critic_main2_network = Critic(state_dim=obs_dim, action_dim=action_dim, hidden_dim=64).to(device)
optimizer_main1_critic = optim.AdamW(critic_main1_network.parameters(), lr=1e-3, weight_decay=0)
optimizer_main2_critic = optim.AdamW(critic_main2_network.parameters(), lr=1e-3, weight_decay=0)

critic_target1_network = copy.deepcopy(critic_main1_network)
critic_target2_network = copy.deepcopy(critic_main2_network)
critic_target1_network.load_state_dict(critic_main1_network.state_dict())
critic_target2_network.load_state_dict(critic_main2_network.state_dict())
        ...
        for _ in range(N_REPLAY_STEPS):
            ...
            if len(memory) > sample_only_until and replay_time_step % update_interval == 0:
                ...
                for epoch in range(N_EPOCHS):
                    ...
                    # comput value 
                    values1 = critic_main1_network(obss_tensor, actions_tensor)
                    values2 = critic_main2_network(obss_tensor, actions_tensor)
                    ...
                    with torch.no_grad():
                        ...
                        next_values1 = critic_target1_network(next_obss_tensor, next_actions).detach()
                        next_values2 = critic_target2_network(next_obss_tensor, next_actions).detach()
                        next_values = torch.min(next_values1, next_values2)
                        
                        td_target = (rewards_tensor + gamma * next_values * (1-dones_tensor))
                    
                    # critic_loss
                    critic_loss = nn.functional.mse_loss(values1, td_target) + nn.functional.mse_loss(values2, td_target)
                    # critic_loss = nn.functional.smooth_l1_loss(value, td_target)
                    
                    # critic_learn
                    optimizer_main1_critic.zero_grad()
                    optimizer_main2_critic.zero_grad()
                    critic_loss.backward()
                    optimizer_main1_critic.step()
                    optimizer_main2_critic.step()
                    
                    # Soft update critic
                    for param, target_param in zip(critic_main1_network.parameters(), critic_target1_network.parameters()):
                        target_param.data.copy_(tau * param.data + (1 - tau) * target_param.data)
                    for param, target_param in zip(critic_main2_network.parameters(), critic_target2_network.parameters()):
                        target_param.data.copy_(tau * param.data + (1 - tau) * target_param.data)
                    
                    if replay_time_step % update_interval_actor == 0:
                        # actor loss
                        actor_loss = -critic_main1_network(obss_tensor, actor_main_network(obss_tensor)).mean()
                ...
                # ---------------------------------------------------------------------------------
                if avg_actor_loss == 0:
                    print(f"\r[{iter+1}/{N_ITER}IT](L: {loop_count}/{total_loop}) Rep: {replay_time_step}/{TOTAL_TIMESTEPS}, Rewards: {np.mean(total_rewards).item():.2f} ([Epochs: {epoch+1}/{N_EPOCHS}] Critic: {avg_critic_loss/N_EPOCHS:.3f})", end='')
                else:
