...
# SARSA Update
def sarsa_update(Q, state, next_state, reward, action, next_action, alpha, gamma):
...
for _ in tqdm(range(num_episodes)):
    ...
    while (done is not True):
        ...
        Q[from_state][action] = sarsa_update(Q, from_state, next_state, reward, action, next_action, alpha, gamma)

        # ★(On-Policy) next action이 다음 step의 action이 되도록
        action = next_action
...
for epoch in tqdm(range(num_episodes)):
    ...
    while (done is not True):
        ...
        next_action, next_q_pred = epsilon_greedy_torch(state=env.cur_state, model=model, epsilon=2e-1, device=device)

        with torch.no_grad():
            model.eval()
            q_values = q_pred.squeeze().clone()

            # SARSA Target :  Gradient Descent가 이미 네트워크의 학습률을 고려하여 가중치를 업데이트하기 때문에 Network학습에서는 SARSA의 완전한 Update식을 따르지 않음
            # q_values[action] = (1-alpha)*q_pred.squeeze()[action] +  alpha * (reward + gamma * next_q_pred.squeeze()[next_action])
            q_values[action] = reward + gamma * next_q_pred.squeeze()[next_action]
        
        # training
        model.train()
        optimizer.zero_grad()
        q_pred = model(torch.tensor(state, dtype=torch.float32).unsqueeze(0).to(device))
