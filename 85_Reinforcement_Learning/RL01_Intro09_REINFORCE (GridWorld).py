...
with tqdm(total=num_episodes, desc=f"Episode {episode_idx+1}/{num_episodes}") as pbar:
    for episode_idx in range(num_episodes):
        ...
        if episode_idx % 1 == 0:
            pbar.set_postfix(Loss=np.mean(policy_loss_list), Num_steps=len(trajectory))
        pbar.update(1)
...
with tqdm(total=num_episodes, desc=f"Episode {episode_idx+1}/{num_episodes}") as pbar:
    for episode_idx in range(num_episodes):
        ...
        policy_loss_list = []
        state_tensors = []
        for (state_str, action, reward), G_t in zip(trajectory, normalized_returns):
            state_tensor = torch.tensor(eval(state_str)).type(torch.float32)
            action_outputs = policy_network(state_tensor)
            action_prob = torch.softmax(action_outputs, dim=-1)
        ...
        if episode_idx % 1 == 0:
            pbar.set_postfix(Loss=policy_loss.item(), Num_steps=len(trajectory))
        pbar.update(1)
