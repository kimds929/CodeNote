...
df.info()
# df['보증_YP하한'] = df['보증_YP하한'].fillna(df['YP_상한']-100)
# df['YP_상한'] = df['YP_상한'].fillna(999)
df['보증_YP하한'] = df['보증_YP하한'].astype('int')
df.head()
...
ggplot(data = df) \
    + geom_hline(yintercept=df[x1], color ='blue', linetype='dashed', alpha=0.2) \
    + geom_hline(yintercept=df[x2], color ='red', linetype='dashed', alpha=0.2) \
    + geom_segment( aes(x='CASE', xend='CASE', y=x1, yend=x2), color ='black', alpha=0.5) \
    + annotate('text', x=df['CASE'], y=df[x1], label=df[x1], size = 8, color ='blue') \
    + annotate('text', x=df['CASE'], y=df[x2], label=df[x2], size = 8, color ='red') \
    + xlab('') + ylab('')\
    + ylim( [df[x1].min(), df[x2].max()] )\
    + theme(axis_text_x = element_text(angle=45) ) \
    + theme_bw()

    # K-Mean : n개의 초기점을 임의로 정하고, 초점간의 유사도를 계산하여 군집으로 묶어줌 (데이터가 많을수록 유사한 군집화 결과가 나옴)
kmean = KMeans(n_clusters = 7, init='random')    # K-Mean Model 생성,  n_cluster : Cluster 갯수
kmean.fit(df_kmean)
...
# df_kmean['kmean'].drop_duplicates()
for i in df_kmean['kmean'].drop_duplicates().tolist():
    df_kmean.loc[df_kmean['kmean']==i, 'kmean']  = 'k' + str(i)     # 항목변경
...
sns.scatterplot(x=x1, y=x2, data=df_kmean, hue="kmean")


df_kmean.to_clipboard(index=False)        #Clipboard로 내보내기
