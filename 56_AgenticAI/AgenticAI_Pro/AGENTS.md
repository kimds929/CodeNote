...
## Must Do Not : 반드시 하지 말아야 할 규칙
 ...
 - **가짜 데이터 금지**
   - 절대 임의의 가짜 데이터(Mock Data)를 생성하지 마세요. 존재하는 `sources/`, `data/` 폴더의 파일만 사용합니다.
 - **임의판단 금지**
...
1. **Phase task 수행 전 : Planning**
	...
	(이후 `Phase {i}`의 모든 Step은 해당 계획에 기반하여 task  수행 예정)
		- Project 목표 : `sources/project_requirements.md`
...
- 상세 내용 : `sources/project_requirements.md` 파일 내 `# MVP1` 파트 확인
- `sources/`내 파일들을 이용하여 `Phase 2`, `Phase 3`에서 LLM 요약 및 ChatBot 서비스 구현을 위한 VectorStoreDB 생성, RAG index 생성
	- `sources/`내에 있는 파일을 읽고 embedding하여 `data/vectorstore/` 폴더를 생성하여 vector저장소(Chroma DB 등)에 저장
...
- 상세 내용 : `sources/project_requirements.md` 파일 내 `# MVP2` 파트 확인
- App service를 위한 `app/main.py`를 생성하고, Streamlit을 이용하여 전체 Layout 구성
- `sources/` 혹은 `data/`내에 있는 필요한 파일들을 읽고 요약정보를 시각화 Dashboard 생성 
   	- csv 파일 load 
...
- 상세 내용 : `sources/project_requirements.md` 파일 내 `# MVP3` 파트 확인
